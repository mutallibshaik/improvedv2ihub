...
"""
    Example of V2I Communication with automatic vehicle control from client side.
"""
...
class HUD(object):
    ...
    def tick(self, world, clock):
        ...
        #print(f"The weather in {location} is {weather_descriptions}, it feels like {feelslike} with wind speed {windspeed} towards {winddir}, humidity {humidity}, clouds {cloud}% and UV is {uv}")
        ##mycodestart
        ...
        #self._info_text += ["ABDUL SHAIKKKKKKK"]
        self._info_text += [
            '',
            'Collision:',
            collision,
       ...
       #     'Speed Limits' % ('(% 5.1f, % 5.1f)'  % (min(liste_2),(world.player.get_speed_limit() )
        ...
        # mqtt_client = mqtt.Client("Client Carla Terminal") #create new instance
        # mqtt_client.connect(broker_address,1883,60) #connect to broker
        # rnum = random.uniform(1.0, 50.0)    
        # if 0 < min(liste) < 20:
        #     rnum = rnum+random.uniform(1.0, 50.0)
        #     mqtt_client.publish("fromcarla", rnum)
        #     print("published from carla:" +str(rnum))
            
        # if min(liste)<5:
        #     #globals()['critical_Flag'] = 1
        #     #print("old condition")
        #     #self.notification_red_zone("You are in Critical Zone "+weatherinfo)        
        #     #self._info_text += pydb2.gettrafficupdates_critical()
            
        if min(liste)<20:
            globals()['critical_Flag'] = 1
            globals()['Range'] = 1
            self.notification_red_zone("You are in Critical Zone ")
            #self.notification_red_zone("You are in Critical Zone" +"The weather is %s in %s, it feels like %2d" % (weather_descriptions, location, feelslike))
            self._info_text += pydb2.gettrafficupdates_critical()
            self._info_text += pydb2.get_carla_atmupdates_critical()
        #elif  20<min(liste)<100: original condition    
        elif  20<min(liste)<50:
            globals()['critical_Flag'] = 0
            globals()['Range'] = 1
            #self.notification_green_zone ("The weather is %s in %s, it feels like %2d" % (weather_descriptions, location, feelslike))
            self.notification_green_zone("Connected to RSU. You are in Safe Zone")
            self._info_text += pydb2.gettrafficupdates_noncritical()
            self._info_text += pydb2.get_carla_atmupdates_noncritical()
        
        elif  min(liste)>50:
            globals()['critical_Flag'] = 0
            globals()['Range'] = 0
            #self.notification_green_zone ("The weather is %s in %s, it feels like %2d" % (weather_descriptions, location, feelslike))
            self.notification_red_zone("Out of RSU Range")
            
   
            
            
            #"Abdul","Shaik","Mutallib"
...
def game_loop(args):
    # rnum = random.uniform(1.0,100.0)

    # client.publish("fromcarla", rnum)
    # print("published from carla:" +str(rnum))
    #client.loop_start()
    pygame.init()
    pygame.font.init()
    ...
    # apply presets
        ...
        while True:
            ...
            # xyz = random.uniform(1,200)
            # mqtt_client.publish("ClientPublished", xyz)
            # print("published: "+str(xyz))
         ...
         #<   world.world.wait_for_tick(10.0)
            ...
            file4=os.path.join("RSU_DATASET", "NORMAL1",world.map.name)
            if not os.path.exists(file4):
                os.makedirs(file4)  
...
def main():
    ...
    broker_address="localhost" 
    client = mqtt.Client("CARLA_SIMULATOR") #create new instance
    ...
    def on_message(client, userdata, msg):  # The callback for when a PUBLISH message is received from the server.
        if(Range == 0):
            print("Out of RSU Range")
        else:
            if(critical_Flag == 1 and msg.topic =="ServerPublished/criticalupdates" ):
                print ("Critical Message received from Server-> " +str(msg.payload.decode("utf-8")))  # Print a received msg
            if(critical_Flag == 0 and msg.topic =="ServerPublished/noncriticalupdates" ):
                print ("Safe Message received from Server-> " +str(msg.payload.decode("utf-8")))  # Print a received msg
            if(critical_Flag == 0 and msg.topic =="ServerPublished/ServerPublished/trafficupdates" ):
                print ("Safe Message received from Server-> " +str(msg.payload.decode("utf-8")))  # Print a received msg
    ...
    try:
        ...
        client.subscribe("ServerPublished/noncriticalupdates")
        client.subscribe("ServerPublished/criticalupdates")         
        game_loop(args)
